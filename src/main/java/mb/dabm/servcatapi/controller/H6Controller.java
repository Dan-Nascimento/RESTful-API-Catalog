...
public class H6Controller {
    ...
    @GetMapping("/inc-classe/fiig/{fiig}")
    @Operation(summary = "Retorna uma lista de registros da tabela INC_CLASSE, " +
        "com paginação, para a coluna FIIG da tabela H6")
    public ResponseEntity<Page<H6DTO>> listAllIncByFiig(
        @PathVariable("fiig") String fiig,
        @RequestParam(value = "page", defaultValue = "0") int page,
        @RequestParam(value = "size", defaultValue = "20") int size
    ) {
        return ResponseEntity.ok(service.finAllIncByFiig(fiig, page, size));
    }
